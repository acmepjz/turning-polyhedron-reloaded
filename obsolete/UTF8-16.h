#pragma once

/** \file
*/

#include <string>

/** UTF-8 encoded string. */
typedef std::string u8string;
/** UTF-16 encoded string. */
typedef std::basic_string<unsigned short> u16string;

/** Convert UTF-16 string to UTF-8 string. */
u8string toUTF8(const u16string& src);
/** Convert UTF-8 string to UTF-16 string. */
u16string toUTF16(const u8string& src);

/** read a Unicode codepoint from a UTF-8 string.
\return bytes read, 0 means error
\warning no sanity check
*/
int readUTF8(const char* src,int& c);

/** read a Unicode codepoint from a UTF-16 string.
\return unsigned shorts read, 0 means error
\warning no sanity check
*/
int readUTF16(const unsigned short* src,int& c);

/** For each character of UTF-8 string, do some operation.

Example:
~~~
u8string s="Hello, World!";
size_t m=s.size();
U8STRING_FOR_EACH_CHARACTER_DO_BEGIN(s,i,m,c,'?');
printf("The character %d is %d\n",i+1,c);
U8STRING_FOR_EACH_CHARACTER_DO_END();
~~~

\param STR The string, e.g. `u8string` or `char*` or something else supports `operator[]`.
\param I The name of index variable.
\param M The size of string.
\param CH The name of character variable.
\param INVALID_CH Invalid UTF-8 characters will be replaced by this character.
*/
#define U8STRING_FOR_EACH_CHARACTER_DO_BEGIN(STR,I,M,CH,INVALID_CH) \
	for(size_t I=0;I<M;I++){ \
		int CH=(unsigned char)STR[I]; \
		if(CH<0x80){ \
		}else if(CH<0xC0){ \
			CH=INVALID_CH; \
		}else if(CH<0xE0){ \
			if(I+1>=M) CH=INVALID_CH; \
			else{ \
				int c2=(unsigned char)STR[I+1]; \
				if((c2&0xC0)!=0x80) CH=INVALID_CH; \
				else{ \
					CH=((CH & 0x1F)<<6) | (c2 & 0x3F); \
					I++; \
				} \
			} \
		}else if(CH<0xF0){ \
			if(I+2>=M) CH=INVALID_CH; \
			else{ \
				int c2=(unsigned char)STR[I+1]; \
				int c3=(unsigned char)STR[I+2]; \
				if((c2&0xC0)!=0x80 || (c3&0xC0)!=0x80) CH=INVALID_CH; \
				else{ \
					CH=((CH & 0xF)<<12) | ((c2 & 0x3F)<<6) | (c3 & 0x3F); \
					I+=2; \
				} \
			} \
		}else if(CH<0xF8){ \
			if(I+3>=M) CH=INVALID_CH; \
			else{ \
				int c2=(unsigned char)STR[I+1]; \
				int c3=(unsigned char)STR[I+2]; \
				int c4=(unsigned char)STR[I+3]; \
				if((c2&0xC0)!=0x80 || (c3&0xC0)!=0x80 || (c4&0xC0)!=0x80) CH=INVALID_CH; \
				else{ \
					CH=((CH & 0x7)<<18) | ((c2 & 0x3F)<<12) | ((c3 & 0x3F)<<6) | (c4 & 0x3F); \
					if(CH>=0x110000) CH=INVALID_CH; \
					else I+=3; \
				} \
			} \
		}else{ \
			CH=INVALID_CH; \
		}

#define U8STRING_FOR_EACH_CHARACTER_DO_END() }

/** Encode a Unicode codepoint to UTF-8 string.

Example:
~~~
const int chs[]={'H','e','l','l','o',',',0x3000,'W','o','r','l','d','!'};
u8string s;
for(int i=0;i<sizeof(chs)/sizeof(chs[0]);i++){
	int ch=chs[i];
	U8_ENCODE(ch,s.push_back);
}
~~~

\param CH The character.
\param OPERATION The operation, e.g. some function name or macro or something else supports `operator()`.
*/
#define U8_ENCODE(CH,OPERATION) \
	if(CH<0x80){ \
		OPERATION(CH); \
	}else if(CH<0x800){ \
		OPERATION(0xC0 | (CH>>6)); \
		OPERATION(0x80 | (CH & 0x3F)); \
	}else if(CH<0x10000){ \
		OPERATION(0xE0 | (CH>>12)); \
		OPERATION(0x80 | ((CH>>6) & 0x3F)); \
		OPERATION(0x80 | (CH & 0x3F)); \
	}else{ \
		OPERATION(0xF0 | (CH>>18)); \
		OPERATION(0x80 | ((CH>>12) & 0x3F)); \
		OPERATION(0x80 | ((CH>>6) & 0x3F)); \
		OPERATION(0x80 | (CH & 0x3F)); \
	}


/** For each character of UTF-16 string, do some operation.

Example:
~~~
u16string s=toUTF16("Hello, World!");
size_t m=s.size();
U16STRING_FOR_EACH_CHARACTER_DO_BEGIN(s,i,m,c,'?');
printf("The character %d is %d\n",i+1,c);
U16STRING_FOR_EACH_CHARACTER_DO_END();
~~~

\param STR The string, e.g. `u16string` or `unsigned short*` or something else supports `operator[]`.
\param I The name of index variable.
\param M The size of string.
\param CH The name of character variable.
\param INVALID_CH Invalid UTF-16 characters will be replaced by this character.
*/
#define U16STRING_FOR_EACH_CHARACTER_DO_BEGIN(STR,I,M,CH,INVALID_CH) \
	for(size_t I=0;I<M;I++){ \
		int CH=(unsigned short)(STR[I]); \
		if(CH<0xD800){ \
		}else if(CH<0xDC00){ \
			/* lead surrogate */ \
			I++; \
			if(I>=M) CH=INVALID_CH; \
			else{ \
				int c2=(unsigned short)STR[I]; \
				if(CH>=0xDC00 && CH<0xE000){ \
					/* trail surrogate */ \
					CH=0x10000 + (((CH & 0x3FF)<<10) | (c2 & 0x3FF)); \
				}else{ \
					/* invalid */ \
					CH=INVALID_CH; \
					I--; \
				} \
			} \
		}else if(CH<0xE000){ \
			/* invalid trail surrogate */ \
			CH=INVALID_CH; \
		}

#define U16STRING_FOR_EACH_CHARACTER_DO_END() }

/** Encode a Unicode codepoint to UTF-16 string.

Example:
~~~
const int chs[]={'H','e','l','l','o',',',0x3000,'W','o','r','l','d','!'};
u16string s;
for(int i=0;i<sizeof(chs)/sizeof(chs[0]);i++){
	int ch=chs[i];
	U16_ENCODE(ch,s.push_back);
}
~~~

\param CH The character.
\param OPERATION The operation, e.g. some function name or macro or something else supports `operator()`.
*/
#define U16_ENCODE(CH,OPERATION) \
	if(CH<0x10000){ \
		OPERATION(CH); \
	}else{ \
		OPERATION(0xD800 | ((CH-0x10000)>>10)); \
		OPERATION(0xDC00 | (CH & 0x3FF)); \
	}
